...
#print(sys.path)
# mtcnn.core
from detect import MtcnnDetector,create_mtcnn_net
from imagedb import ImageDB
from image_reader import TestImageLoader
from core_utils import convert_to_square, IoU
...
prefix_path = os.path.join(ROOT_DIR, 'data_set', 'face_detection', 'WIDER_train', 'images')
traindata_store = os.path.join(ROOT_DIR, 'data_set', 'train')
pnet_model_file = os.path.join(ROOT_DIR, 'model_store', 'pnet_epoch_30.pt')
annotation_file = os.path.join(ROOT_DIR, 'data_set', 'face_detection', 'wider_face_split', 'wider_face_train_mtcnn_bbx.txt')
...
def gen_rnet_data(data_dir, anno_file, pnet_model_file, prefix_path='', use_cuda=True, vis=False):
    ...
    # './model_store'
    save_path = os.path.join(ROOT_DIR, 'model_store')
...
def gen_rnet_sample_data(data_dir, anno_file, det_boxs_file, prefix_path):
    ...
    # './anno_store'
    save_path = os.path.join(ROOT_DIR, 'anno_store')
    if not os.path.exists(save_path):
        ...
        for box in dets:
            ...
            if np.max(Iou) < 0.3 and neg_num < 60:
                ...
                if np.max(Iou) >= 0.65:
                    ...
                    d_idx += 1
        box_idx += 1
        print("%s images done, pos: %s part: %s neg: %s" % (box_idx, p_idx, d_idx, n_idx))
    f1.close()
    f2.close()
    f3.close()
