...
def generate_data(num_samples, num_inputs, num_outputs, num_outliers):
    ...
    if True: # Add outliers
        ...
        A_noisy[outlier_idxs] = 20 * np.random.random((num_outliers, num_inputs)) # Add noise and outliers Xi
        B_noisy[outlier_idxs] = np.random.normal(size=(num_outliers, num_outputs)) # Add noise and outliers Yi
